        ...
        <div className="grid md:grid-cols-2 gap-4">
          {products.map(p => (
            <div key={p.id} className="border border-zinc-800 rounded-xl p-4 bg-zinc-950 flex gap-4">
              ...
              <div><div className="font-semibold text-amber-300">{p.name}</div><div className="text-xs text-zinc-500">{p.category}</div><div className="text-sm font-bold mt-1">{p.price?.toLocaleString()} ETB</div><div className="text-xs mt-1">{p.in_stock ? '🟢 In stock' : '🔴 Out of stock'} {p.is_featured && '• ⭐ Featured'}</div></div>
            </div>
          ))}
        </div>
      )}
